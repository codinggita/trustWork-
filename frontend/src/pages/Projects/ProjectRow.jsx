const ProjectRow = ({ project }) => {
  const getDeadlineInfo = (project) => {
    ...
    // Generate some deterministic but relative deadlines based on ID
    const diff = (project.id % 7) - 2; 
  ...
  return (
    ...
    >
      ...
      {/* RIGHT: Status, Amount, Deadline, Action */}
      <div className="flex flex-wrap md:flex-nowrap items-center justify-between md:justify-end gap-6 md:w-2/5 mt-4 md:mt-0">
        <div className="text-right min-w-[120px]">
          <p className="text-[16px] font-extrabold text-gray-900">{project.amount}</p>
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); alert(`Opening Workspace for: ${project.name}`); }}
            className="px-5 py-2.5 bg-gray-50 hover:bg-[#6C5CE7] hover:text-white text-[#6C5CE7] text-[13px] font-bold rounded-xl border border-gray-100 transition-all active:scale-95 hover:shadow-lg hover:shadow-[#6C5CE7]/20"
          >
            View Workspace
          </button>
        </div>
